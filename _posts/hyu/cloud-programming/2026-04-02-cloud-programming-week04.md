...
클라우드 환경에서는 애플리케이션을 한 번 실행하고 끝내는 것이 아니라, 여러 환경에 반복적으로 배포해야 한다. 이때 가장 많이 나오는 문제가 “내 컴퓨터에서는 되는데 서버에서는 안 된다”는 문제다.

Docker는 애플리케이션과 실행 환경을 이미지로 묶고, 그 이미지를 컨테이너로 실행한다. 개발 환경, 테스트 환경, 운영 환경의 차이를 줄이기 위해 등장한 도구라고 볼 수 있다.

따라서 Docker를 배운다는 것은 단순히 명령어 몇 개를 외우는 것이 아니라, 애플리케이션 실행 환경을 코드처럼 다루는 방식을 배우는 것이다.
